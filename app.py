...
def preguntar():
    ...
    try:
        ...
        if resultado.get('intent') == 'get_sql' and pregunta.lower().startswith('dame la consulta'):
            return jsonify({"sql_query": resultado['sql'], "suggestions": resultado.get('suggestions', [])})
            ...
            return jsonify({"response": resultado['respuesta'], "chart_data": chart_data, "suggestions": resultado.get('suggestions', [])})
        return jsonify({
